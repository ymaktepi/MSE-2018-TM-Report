...
bigger the score for number of nodes is, the higher the error.  \todo{explain
why 2.5; number of nodes is the one with lower variance therefore the linear
regression fits using only it as a preponderant variable. the mean number of
nodes is roughly 10.5. when fitting the model, it is therefore reduced to \(1 =
s_n/n = 2.5 / \)}

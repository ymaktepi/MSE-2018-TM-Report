...
\subsection{Alternative visualizations}
...
data points. A picture of the visualization is showed in \fig{fig:3Dplot}. As the
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.7\columnwidth]{3D_arbitrary_20_nodes_4_depth}
...
to the number of nodes for multiple strategies. \fig{fig:commonViz} shows all
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\columnwidth]{tendency_20_nodes_4_depth_all_receivers}
    ...
    in \ssec{ssec:arbitrary}. The Maximal Overhead strategy plot (red) shows a
...
As seen at the end of \ssec{ssec:summaryViz}, the current plots only offer a

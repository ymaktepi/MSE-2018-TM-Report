...
on the other hand should be quite high because as seen in \ssec{ssec:cbc},
\(50\%\) of the validators have to acknowledge the messages to finalize them and
since the round-robin has a fixed rotation order for the message production,
this only happens after making one and a half rotations (the first one to
acknoledge the messages existance and the next half to confirm the
acknoledgment.

...
\fig{fig:example} demonstrates a small example of a \gls{cbc} execution oer a
...
A block is considered safe when every node in a subset of the network has seen
each other member of the same subset agreeing on said block. Two nodes agree on
a block when they both include it in the blockchain of their estimate. In this
project, the subset will be any 50\% of the network.  }
...
TrueLevel \todo{introduce somewhere}. It implements the consensus algorithms
